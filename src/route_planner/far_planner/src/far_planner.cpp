...
void FARMaster::Init() {
  ...
  main_event_     = nh_->create_wall_timer(std::chrono::milliseconds(int(duration_time * 1000)), std::bind(&FARMaster::MainLoopCallBack, this));// 负责更新机器人位置、地图、导航图结构，并进行可视化
  planning_event_ = nh_->create_wall_timer(std::chrono::milliseconds(int(duration_time * 1000)), std::bind(&FARMaster::PlanningCallBack, this));// 负责实际路径搜索与目标点发布，将全局路径分解为局部目标点，通过 /way_point 话题发布给 local_planner。
...
void FARMaster::MainLoopCallBack() {
  ...
  if (!this->PreconditionCheck()) {
      RCLCPP_WARN(nh_->get_logger(), "调试: PreconditionCheck失败，跳过本次循环");
      return;
  }
  RCLCPP_INFO(nh_->get_logger(), "调试: PreconditionCheck通过");
  ...
  RCLCPP_INFO(nh_->get_logger(), "调试: odom_node_ptr_已初始化");
  
  /* Extract Vertices and new nodes */
  FARUtil::Timer.start_time("Total V-Graph Update");
  // RCLCPP_INFO(nh_->get_logger(), "调试: surround_obs_cloud_大小: %zu", FARUtil::surround_obs_cloud_->size());
  contour_detector_.BuildTerrainImgAndExtractContour(odom_node_ptr_, FARUtil::surround_obs_cloud_, realworld_contour_);
  // RCLCPP_INFO(nh_->get_logger(), "调试: realworld_contour_大小: %zu", realworld_contour_.size());
  ...
  // RCLCPP_INFO(nh_->get_logger(), "调试: contour_graph_大小: %zu", ContourGraph::contour_graph_.size());
  ...
  // RCLCPP_INFO(nh_->get_logger(), "调试: new_ctnodes_大小: %zu", new_ctnodes_.size());
  ...
  if (!is_stop_update_ && graph_manager_.ExtractGraphNodes(new_ctnodes_)) {
    ...
    RCLCPP_INFO(nh_->get_logger(), "调试: ExtractGraphNodes成功, new_nodes_大小: %zu", new_nodes_.size());
  } else {
    RCLCPP_INFO(nh_->get_logger(), "调试: ExtractGraphNodes失败或被跳过");
  }
  if (is_graph_init_) {
    if (!FARUtil::IsDebug) printf("\033[2K");
  ...
  // RCLCPP_INFO(nh_->get_logger(), "调试: nav_graph_大小: %zu", nav_graph_.size());
  if (is_graph_init_) {
    if (!FARUtil::IsDebug) printf("\033[2K");
  ...
  if (!is_graph_init_ && !nav_graph_.empty()) {
    ...
    RCLCPP_INFO(nh_->get_logger(), "调试: V-Graph初始化成功！");
  } else if (!is_graph_init_) {
    RCLCPP_WARN(nh_->get_logger(), "调试: V-Graph仍未初始化, nav_graph_大小: %zu", nav_graph_.size());
  }

}

void FARMaster::PlanningCallBack() {
  // RCLCPP_INFO(nh_->get_logger(), "====== PlanningCallBack DEBUG ======");
  
  if (!is_init_completed_ || !is_graph_init_) {
    RCLCPP_INFO(nh_->get_logger(), "Planning skipped: init_completed=%d, graph_init=%d", 
                is_init_completed_, is_graph_init_);
    return;
  }
  
  const NavNodePtr goal_ptr = graph_planner_.GetGoalNodePtr();
  RCLCPP_INFO(nh_->get_logger(), "Goal node pointer: %s", (goal_ptr == NULL) ? "NULL" : "Valid");
  ...
  if (goal_ptr == NULL) {
    ...
    RCLCPP_INFO(nh_->get_logger(), "No goal 6, only updating graph traversability");
  } else { 
    // Update goal postion with nearby terrain cloud
    const Point3D ori_p = graph_planner_.GetOriginNodePos(true);
    RCLCPP_INFO(nh_->get_logger(), "Origin node position: x=%.3f, y=%.3f, z=%.3f", 
                ori_p.x, ori_p.y, ori_p.z);
    ...
    RCLCPP_INFO(nh_->get_logger(), "Got cloud points around goal: obs=%zu, free=%zu", 
                goal_obs->size(), goal_free->size());
    ...
    RCLCPP_INFO(nh_->get_logger(), "Re-evaluated goal position: x=%.3f, y=%.3f, z=%.3f", 
    ...
    if (!FARUtil::IsDebug) printf("\033[2K");
    float addGoalTime = FARUtil::Timer.end_time("Adding Goal to V-Graph");
    RCLCPP_INFO(nh_->get_logger(), "Goal added to V-Graph in %.3f ms", addGoalTime);
    ...
    RCLCPP_INFO(nh_->get_logger(), "Graph traversability updated");
    ...
    RCLCPP_INFO(nh_->get_logger(), "Starting path planning to goal...");
    bool pathResult = graph_planner_.PathToGoal(goal_ptr, global_path, nav_node_ptr_, 
                                              current_free_goal, is_planning_fails, 
                                              is_reach_goal, is_current_free_nav);
    RCLCPP_INFO(nh_->get_logger(), "Path planning result: %s", pathResult ? "SUCCESS" : "FAILED");
    RCLCPP_INFO(nh_->get_logger(), "Planning status: fails=%d, reach_goal=%d, free_nav=%d, nav_node=%s", 
                is_planning_fails, is_reach_goal, is_current_free_nav, 
                (nav_node_ptr_ != NULL) ? "Valid" : "NULL");
    
    if (pathResult && nav_node_ptr_ != NULL) {
      RCLCPP_INFO(nh_->get_logger(), "Valid path found with %zu waypoints", global_path.size());
      Point3D waypoint = nav_node_ptr_->position;
      RCLCPP_INFO(nh_->get_logger(), "Initial waypoint: x=%.3f, y=%.3f, z=%.3f", 
                  waypoint.x, waypoint.y, waypoint.z);
      
      if (nav_node_ptr_ != goal_ptr) {
        waypoint = this->ProjectNavWaypoint(nav_node_ptr_, last_nav_ptr);
        RCLCPP_INFO(nh_->get_logger(), "Projected waypoint: x=%.3f, y=%.3f, z=%.3f", 
                    waypoint.x, waypoint.y, waypoint.z);
      } else if (master_params_.is_viewpoint_extend) {
        planner_viz_.VizViewpointExtend(goal_ptr, goal_ptr->position);
        RCLCPP_INFO(nh_->get_logger(), "Using goal as waypoint, with viewpoint extension");
      }
      
      goal_waypoint_stamped_.point = FARUtil::Point3DToGeoMsgPoint(waypoint);
      goal_pub_->publish(goal_waypoint_stamped_);
      RCLCPP_INFO(nh_->get_logger(), "Published waypoint to /way_point: x=%.3f, y=%.3f, z=%.3f",
                  goal_waypoint_stamped_.point.x, goal_waypoint_stamped_.point.y, 
                  goal_waypoint_stamped_.point.z);
      ...
      if (is_planning_fails) { // stops the robot
        goal_waypoint_stamped_.point = FARUtil::Point3DToGeoMsgPoint(robot_pos_);
        goal_pub_->publish(goal_waypoint_stamped_);
        RCLCPP_INFO(nh_->get_logger(), "Planning failed, stopping robot at current position: x=%.3f, y=%.3f, z=%.3f",
                    robot_pos_.x, robot_pos_.y, robot_pos_.z);
      } else {
        RCLCPP_INFO(nh_->get_logger(), "Planner stopping (no new waypoint)");
      }
    } else {
      RCLCPP_INFO(nh_->get_logger(), "No valid path found and planner not running");
    }
    if (!FARUtil::IsDebug) printf("\033[2K");
    ...
    if (is_reach_goal) {
      ...
      RCLCPP_INFO(nh_->get_logger(), "Goal reached! Execution completed.");
    }

    plan_timer_.data = FARUtil::Timer.end_time("Path Search");
    planning_time_pub_->publish(plan_timer_);
    RCLCPP_INFO(nh_->get_logger(), "Path search time: %.3f ms", plan_timer_.data);
  }
  RCLCPP_INFO(nh_->get_logger(), "====== PlanningCallBack End ======\n");
}
...
void FARMaster::LoadROSParams() {
  ...
  nh_->declare_parameter<float>("voxel_dim", 0.1);
  nh_->declare_parameter<float>("robot_dim", 0.62);
  nh_->declare_parameter<float>("vehicle_height", 0.45);
  nh_->declare_parameter<float>("sensor_range", 10.0);
  nh_->declare_parameter<float>("terrain_range", 10.0); 
  nh_->declare_parameter<float>("local_planner_range", 3.0);
  ...
  nh_->declare_parameter<bool>("is_static_env", false);
...
void FARMaster::OdomCallBack(const nav_msgs::msg::Odometry::SharedPtr msg) {
  ...
  // RCLCPP_INFO(nh_->get_logger(), "-------------------- OdomCallBack triggered --------------------");
  // RCLCPP_INFO(nh_->get_logger(), "Received odometry message with timestamp: %.4f", msg->header.stamp.sec + msg->header.stamp.nanosec * 1e-9);
  // RCLCPP_INFO(nh_->get_logger(), "Odometry header frame_id: '%s'", msg->header.frame_id.c_str());
  // RCLCPP_INFO(nh_->get_logger(), "Odometry child_frame_id: '%s'", msg->child_frame_id.c_str());
  ...
  if (!FARUtil::IsSameFrameID(odom_frame, master_params_.world_frame)) {
    ...
    try
    {
      rclcpp::Time lookup_time = msg->header.stamp;
      // RCLCPP_INFO(nh_->get_logger(), "Requesting TF lookup at time: %.4f", lookup_time.seconds());
...
void FARMaster::WaypointCallBack(const geometry_msgs::msg::PointStamped& route_goal) {
  // DEBUG: Print received goal point
  // RCLCPP_INFO(nh_->get_logger(), "====== WaypointCallBack DEBUG ======");
  // RCLCPP_INFO(nh_->get_logger(), "Received goal point: x=%.3f, y=%.3f, z=%.3f", 
  //             route_goal.point.x, route_goal.point.y, route_goal.point.z);
  // RCLCPP_INFO(nh_->get_logger(), "Frame ID: %s", route_goal.header.frame_id.c_str());
  ...
  if (!FARUtil::IsSameFrameID(goal_frame, master_params_.world_frame)) {
    ...
    RCLCPP_INFO(nh_->get_logger(), "Transforming from frame '%s' to '%s'", 
  ...
  // DEBUG: Print transformed goal point
  // RCLCPP_INFO(nh_->get_logger(), "After transform - Goal point: x=%.3f, y=%.3f, z=%.3f", 
  //             goal_p.x, goal_p.y, goal_p.z);
              
  graph_planner_.UpdateGoal(goal_p);
  RCLCPP_INFO(nh_->get_logger(), "Goal point updated in graph planner");
  ...
  RCLCPP_INFO(nh_->get_logger(), "====== WaypointCallBack End ======\n");
}
