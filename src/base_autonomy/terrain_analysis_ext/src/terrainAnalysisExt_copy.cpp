...
{
  RCUTILS_LOG_INFO("odometryHandler start\n");
  ...
  RCUTILS_LOG_INFO("odometryHandler end\n");
}

// registered laser scan callback function
void laserCloudHandler(const sensor_msgs::msg::PointCloud2::ConstSharedPtr laserCloud2)
{
  RCUTILS_LOG_INFO("laserCloudHandler start\n");
  ...
  newlaserCloud = true;
  RCUTILS_LOG_INFO("laserCloudHandler end\n");
}
...
{
  RCUTILS_LOG_INFO("terrainCloudLocalHandler start\n");
  terrainCloudLocal->clear();
  pcl::fromROSMsg(*terrainCloudLocal2, *terrainCloudLocal);
  RCUTILS_LOG_INFO("terrainCloudLocalHandler end\n");
}

// joystick callback function
void joystickHandler(const sensor_msgs::msg::Joy::ConstSharedPtr joy)
{
  RCUTILS_LOG_INFO("joystickHandler start\n");
  
  RCUTILS_LOG_INFO("joystickHandler end\n");
}

// cloud clearing callback function
void clearingHandler(const std_msgs::msg::Float32::ConstSharedPtr dis)
{
  RCUTILS_LOG_INFO("clearingHandler start\n");
  clearingDis = dis->data;
  clearingCloud = true;
  RCUTILS_LOG_INFO("clearingHandler end\n");
}
...
int main(int argc, char** argv)
{
  ...
  RCLCPP_INFO(nh->get_logger(),"11111");
  ...
  RCLCPP_INFO(nh->get_logger(),"111222222");
  while (status)
  {
    rclcpp::spin_some(nh);
    RCLCPP_INFO(nh->get_logger(),"status ssassdasdsadsadsad");
    RCLCPP_INFO(nh->get_logger(), "Node address: %p", (void*)nh.get());  // 如果是智能指针
    if (newlaserCloud)
    {
      cnt ++;
      RCLCPP_INFO(nh->get_logger(),"cnt is:%lld",cnt);
      newlaserCloud = false;
      ...
      RCLCPP_INFO(nh->get_logger(),"22222");
      ...
      RCLCPP_INFO(nh->get_logger(),"33333");
      ...
      RCLCPP_INFO(nh->get_logger(),"444444");
      if (useSorting)
      {
        for (int i = 0; i < planarVoxelNum; i++)
        {
      ...
      RCLCPP_INFO(nh->get_logger(),"555555");
      ...
      RCLCPP_INFO(nh->get_logger(),"66666");
      ...
      RCLCPP_INFO(nh->get_logger(),"777777");
    }
