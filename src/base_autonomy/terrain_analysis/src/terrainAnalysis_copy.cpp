...
double vehicleHeight = 0.44;  // 车辆高度 44cm -> 0.44m
int voxelPointUpdateThre = 100;
double voxelTimeUpdateThre = 2.0;
double minRelZ = -0.6;        // 最低有效点高度（原-1.5）
double maxRelZ =  0.3;        // 最高地面点高度（原0.2
double disRatioZ = 0.15;      // 距离对高度的影响系数（原0.2）
...
void imuHandler(const sensor_msgs::msg::Imu::ConstSharedPtr imu)
{
  ...
  geometry_msgs::msg::Quaternion geoQuat = imu->orientation;
...
int main(int argc, char **argv) {
  ...
  auto subOdometry = nh->create_subscription<nav_msgs::msg::Odometry>(
    "/tita4264886/chassis/odometry", 5, odometryHandler);
  auto subImu = nh->create_subscription<sensor_msgs::msg::Imu>(
    "/tita4264886/imu_sensor_broadcaster/imu", rclcpp::SensorDataQoS(), imuHandler);
  ...
  while (status) {
    ...
    if (newlaserCloud) {
      ...
      RCLCPP_INFO(nh->get_logger(), "After crop: %d", laserCloudCropSize);
      RCLCPP_INFO(nh->get_logger(), "Terrain cloud: %d", terrainCloudSize);
      RCLCPP_INFO(nh->get_logger(), "Output points: %d", terrainCloudElevSize);
